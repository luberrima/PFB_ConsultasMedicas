...
export const initDb = async () => {
    try {
        ...
        await pool.query(
            'DROP TABLE IF EXISTS replys, documents, consultation, doctors,users,skills;'
        );
...
y no se podra selecionar Se deberia evaluar que en el caso de endpoint de borrado de una skill este forzado a seleccionar otra para que todos los datos sean pasado a la nueva skill*/
  ...
  */
    ...
    `);
        ...
        /* Crear tabla consultation el campo diagnostico es el campo para finalizar la consulta y 
  ...
  */
    ...
    `);
      ...
      CREATE TABLE replys (
        id CHAR(36) PRIMARY KEY NOT NULL,
        reply TEXT NOT NULL,
	      consultationId CHAR(36) NOT NULL,
    ...
    `);
        ...
        await Promise.all(
            skills.map((skill) =>
                pool.query(`INSERT INTO skills (name) VALUES (?)`, skill)
            )
        );
